...
import type { MeetingHooks } from "./meeting.hooks"
...
/**
 * Set hooks for Meeting routes
 */
export function setMeetingHooks(h: MeetingHooks) {
  hooks = h
}
...
const WRITABLE_SCALAR_FIELDS = ["title", "date", "duration", "notes", "summary", "actionItems", "source", "createdAt", "updatedAt", "userId", "clientId", "projectId"] as const
const RELATION_FIELDS = ["user", "client", "project", "tasks"] as const
...
/**
 * Create Meeting CRUD routes
 */
export function createMeetingRoutes(): Hono {
  ...
  router.get("/:id", async (c) => {
    try {
      ...
      if (!item) {
        return sendJson(c, { error: { code: "not_found", message: "Meeting not found" } }, 404)
      }
  ...
  router.patch("/:id", async (c) => {
    try {
      ...
      const item = await prisma.meeting.update({
        where: { id },
        data: picked.data,
      })
