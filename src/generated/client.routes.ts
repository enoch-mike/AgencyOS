...
/**
 * Set hooks for Client routes
 */
export function setClientHooks(h: ClientHooks) {
  hooks = h
}
...
const WRITABLE_SCALAR_FIELDS = ["name", "email", "company", "industry", "status", "notes", "aiInsights", "healthScore", "lastContact", "createdAt", "updatedAt", "userId"] as const
const RELATION_FIELDS = ["user", "projects", "meetings", "contacts"] as const
...
export function createClientRoutes(): Hono {
  ...
  router.get("/:id", async (c) => {
    try {
      ...
      if (!item) {
        return sendJson(c, { error: { code: "not_found", message: "Client not found" } }, 404)
      }
