...
import type { FileHooks } from "./file.hooks"
...
/**
 * Set hooks for File routes
 */
export function setFileHooks(h: FileHooks) {
  hooks = h
}
...
const WRITABLE_SCALAR_FIELDS = ["name", "url", "type", "size", "createdAt", "projectId"] as const
...
export function createFileRoutes(): Hono {
  ...
  router.get("/:id", async (c) => {
    try {
      ...
      if (!item) {
        return sendJson(c, { error: { code: "not_found", message: "File not found" } }, 404)
      }
  ...
  router.patch("/:id", async (c) => {
    try {
      ...
      const item = await prisma.file.update({
        where: { id },
        data: picked.data,
      })
