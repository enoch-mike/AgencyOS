...
import type { TaskHooks } from "./task.hooks"
...
/**
 * Set hooks for Task routes
 */
export function setTaskHooks(h: TaskHooks) {
  hooks = h
}
...
const WRITABLE_SCALAR_FIELDS = ["title", "description", "status", "priority", "dueDate", "assignedTo", "source", "createdAt", "updatedAt", "userId", "projectId", "meetingId"] as const
const RELATION_FIELDS = ["user", "project", "meeting", "tags"] as const
...
export function createTaskRoutes(): Hono {
  ...
  router.get("/:id", async (c) => {
    try {
      ...
      if (!item) {
        return sendJson(c, { error: { code: "not_found", message: "Task not found" } }, 404)
      }
