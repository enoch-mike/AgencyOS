...
import type { ActivityLogHooks } from "./activity-log.hooks"
...
/**
 * Set hooks for ActivityLog routes
 */
export function setActivityLogHooks(h: ActivityLogHooks) {
  hooks = h
}
...
const WRITABLE_SCALAR_FIELDS = ["action", "entityType", "entityId", "details", "createdAt", "userId"] as const
...
/**
 * Create ActivityLog CRUD routes
 */
export function createActivityLogRoutes(): Hono {
  ...
  router.get("/:id", async (c) => {
    try {
      ...
      const item = await prisma.activityLog.findUnique({
        where: { id },
      })

      if (!item) {
        return sendJson(c, { error: { code: "not_found", message: "ActivityLog not found" } }, 404)
      }
  ...
  router.post("/", async (c) => {
    try {
      ...
      console.error("[ActivityLog] Create error:", error)
  ...
  router.patch("/:id", async (c) => {
    try {
      ...
      console.error("[ActivityLog] Update error:", error)
