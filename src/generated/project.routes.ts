...
const WRITABLE_SCALAR_FIELDS = ["name", "description", "status", "priority", "deadline", "budget", "spent", "progress", "createdAt", "updatedAt", "userId", "clientId"] as const
const RELATION_FIELDS = ["user", "client", "tasks", "meetings", "milestones", "files"] as const
...
export function createProjectRoutes(): Hono {
  ...
  router.get("/:id", async (c) => {
    try {
      ...
      if (!item) {
        return sendJson(c, { error: { code: "not_found", message: "Project not found" } }, 404)
      }
